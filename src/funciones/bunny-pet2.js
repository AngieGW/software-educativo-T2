...
(function() {
    ...
    // Frases motivacionales sobre recordar lo aprendido
    const recallPhrases = [
        "¡Confía en tu memoria, sabes más de lo que crees! 🧠",
        "Recordar es parte del aprendizaje, ¡inténtalo antes de buscar la respuesta!",
        "¡Haz un esfuerzo por recordar, tu cerebro se fortalece con cada intento!",
        "Repasar mentalmente lo aprendido te ayuda a retenerlo mejor.",
        "¡No te preocupes si no recuerdas todo a la primera, sigue intentando!",
        "Cada vez que recuerdas, tu conocimiento se vuelve más sólido.",
        "¡Recordar es como ejercitar un músculo, entre más lo uses, más fuerte será!",
        "¡Atrévete a recordar antes de consultar tus apuntes!",
        "El esfuerzo de recordar es tan valioso como aprender algo nuevo.",
        "¡Confía en ti, tu mente está llena de respuestas!"
    ];
    ...
    // Función para mostrar frase de recordar
    function showRecallPhrase(speechBubble, bunnyImg, idx = null) {
        const index = idx !== null ? idx % recallPhrases.length : Math.floor(Math.random() * recallPhrases.length);
        speechBubble.textContent = recallPhrases[index];
        speechBubble.style.display = 'block';
        bunnyImg.classList.add('bounce');
        setTimeout(() => {
            bunnyImg.classList.remove('bounce');
    ...
    function initializeBunny() {
        ...
        function startRotation() {
            ...
            rotationInterval = setInterval(() => {
                ...
                // Alternar entre frases de lectura y de recordar
                if (bunnyIdx % 2 === 0) {
                    showReadingPhrase(speechBubble, bunnyImg, bunnyIdx);
                } else {
                    showRecallPhrase(speechBubble, bunnyImg, bunnyIdx);
                }
            }, 60000);

            // Mostrar frases automáticamente cada 30 segundos (puedes alternar aquí también si quieres)
            phraseInterval = setInterval(() => {
                // Alternar entre motivacionales y de recordar
                if (Math.random() < 0.5) {
                    showRandomPhrase(speechBubble, bunnyImg);
                } else {
                    showRecallPhrase(speechBubble, bunnyImg);
                }
            }, 30000);
        }
