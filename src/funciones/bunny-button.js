...
(function() {
    ...
    function showCustomMessage(speechBubble, bunnyButton, message, duration = 10000) {
    ...
    function initializeBunny() {
        ...
        // --- NUEVO: Control de clicks rápidos ---
        let clickTimes = [];

        bunnyButton.addEventListener('click', function() {
            const now = Date.now();
            // Filtrar clicks en los últimos 10 segundos
            clickTimes = clickTimes.filter(ts => now - ts < 10000);
            clickTimes.push(now);

            if (clickTimes.length >= 5) {
                showCustomMessage(speechBubble, bunnyButton, "Me estás lastimando", 10000);
                clickTimes = []; // Reinicia el contador para evitar spam
            } else {
                showRandomPhrase(speechBubble, bunnyButton);
            }
        });

        // --- NUEVO: Mensaje de presentación al recargar la página ---
        setTimeout(() => {
            showCustomMessage(
                speechBubble,
                bunnyButton,
