...
async function getPreguntasProyecto1Contenido2() {
    ...
    return mezclarArray(
        data.data.filter(p => p.id_proyecto === 1 && p.id_contenido === 1)
    );
}
...
// Exporta la función para obtener el puntaje actual
window.getPuntajeEvaluacion = function() {
    return puntaje;
};

// Detecta si se muestra un alert y ejecuta una función extra (por ejemplo, mostrar bunny)
(function() {
    const originalAlert = window.alert;
    window.alert = function(msg) {
        // Llama al alert original
        originalAlert.apply(window, arguments);
        // Si existe la función mostrarResultadoBunny, la ejecuta con el puntaje actual
        if (window.mostrarResultadoBunny) {
            window.mostrarResultadoBunny(window.getPuntajeEvaluacion());
        }
    };
})();